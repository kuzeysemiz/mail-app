...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return <span className={cn("inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium border", cfg[status] || "bg-secondary text-muted-foreground border-border")}>{lbl[status] || status}</span>;
};
...
export default function EmailManager() {
  ...
  return (
    <div className="space-y-5">
      {msg.text && (
        <div className={`px-4 py-3 rounded-lg text-sm border ${msg.type === "success" ? "bg-primary/10 text-primary border-primary/20" : "bg-destructive/10 text-destructive border-destructive/20"}`}>{msg.text}</div>
      )}
      ...
      <div className="bg-card border border-border rounded-xl p-4">
        <div className="flex flex-wrap items-center gap-3">
          <select value={selected} onChange={e => setSelected(e.target.value)}
            className="flex-1 min-w-0 px-3 py-2 bg-input border border-border rounded-lg text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-ring">
          ...
          {viewMode === "emails" && (
            <button onClick={() => { setViewMode("batches"); setSelectedBatch(null); loadBatches(); }}
              className="flex items-center gap-1.5 px-3 py-2 bg-secondary border border-border text-sm text-foreground rounded-lg hover:bg-muted transition-colors">
              <ChevronLeft size={15} />Listelere Dön
            </button>
          )}

          <button onClick={() => viewMode === "batches" ? loadBatches() : loadEmails(selectedBatch?.batchId)}
            className="p-2 bg-secondary border border-border text-muted-foreground rounded-lg hover:bg-muted transition-colors">
            <RefreshCw size={15} />
          </button>

          <label className="flex items-center gap-2 text-sm text-muted-foreground cursor-pointer whitespace-nowrap">
            <input type="checkbox" checked={sendOverdue} onChange={async e => { setSendOverdue(e.target.checked); await settingsAPI.set("send_overdue", String(e.target.checked)); }} className="accent-primary" />
        ...
        <div className="space-y-3">
          {batches.length === 0 && (
            <div className="text-center py-16 text-muted-foreground bg-card border border-border rounded-xl">
              <List size={36} className="mx-auto mb-3 opacity-20" /><p className="text-sm">Henüz email listesi yok</p>
            </div>
          )}
          {batches.map(batch => (
            <div key={batch.batchId} className="bg-card border border-border rounded-xl p-5">
              {editingBatchId === batch.batchId ? (
                <div className="space-y-3">
                  <input value={editBatchData.mailSubject} onChange={e => setEditBatchData(p => ({...p, mailSubject: e.target.value}))}
                    className="w-full px-3 py-2 bg-input border border-border rounded-lg text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-ring" placeholder="Başlık" />
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => handleSaveBatchEdit(batch.batchId)} className="px-3 py-1.5 bg-primary text-[oklch(0.11_0.005_260)] rounded-lg text-sm font-semibold hover:opacity-90">Kaydet</button>
                    <button onClick={() => setEditingBatchId(null)} className="px-3 py-1.5 bg-secondary border border-border text-foreground rounded-lg text-sm hover:bg-muted">İptal</button>
                  </div>
                </div>
              ) : (
                    ...
                    {renamingBatchId === batch.batchId ? (
                      <div className="flex gap-2 mb-2">
                        <input value={renameValue} onChange={e => setRenameValue(e.target.value)} autoFocus
                          className="px-2 py-1 bg-input border border-border rounded-lg text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-ring" />
                        <button onClick={() => handleRename(batch.batchId)} className="px-2 py-1 bg-primary text-[oklch(0.11_0.005_260)] rounded-lg text-xs font-semibold">✓</button>
                        <button onClick={() => setRenamingBatchId(null)} className="px-2 py-1 bg-secondary border border-border rounded-lg text-xs">✕</button>
                      </div>
                    ) : (
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <button onClick={() => { setRenamingBatchId(batch.batchId); setRenameValue(batch.mailSubject || ""); }}
                          className="p-1 text-muted-foreground hover:text-foreground rounded transition-colors"><Edit2 size={12} /></button>
                      </div>
                    )}
                    <p className="text-xs text-muted-foreground mb-3">{new Date(batch.createdAt).toLocaleDateString("tr-TR")}</p>
                    <div className="flex flex-wrap gap-2">
                      <span className="text-xs px-2 py-0.5 bg-secondary rounded-full text-muted-foreground">Toplam: {batch.totalCount}</span>
                      {batch.sentCount > 0    && <span className="text-xs px-2 py-0.5 bg-primary/10 text-primary rounded-full">Gönderildi: {batch.sentCount}</span>}
                      {batch.pendingCount > 0 && <span className="text-xs px-2 py-0.5 bg-yellow-500/10 text-yellow-400 rounded-full">Bekliyor: {batch.pendingCount}</span>}
                      {batch.failedCount > 0  && <span className="text-xs px-2 py-0.5 bg-destructive/10 text-destructive rounded-full">Başarısız: {batch.failedCount}</span>}
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2 shrink-0">
                    <button onClick={() => openBatch(batch)} className="flex items-center gap-1 px-3 py-1.5 bg-secondary border border-border text-sm text-foreground rounded-lg hover:bg-muted transition-colors"><LayoutGrid size={13} />Detaylar</button>
                    {batch.pendingCount > 0 && <button onClick={() => handleSendBatchNow(batch.batchId)} className="flex items-center gap-1 px-3 py-1.5 bg-primary/10 text-primary border border-primary/20 rounded-lg text-sm hover:bg-primary/20 transition-colors"><Zap size={13} />Hemen Gönder</button>}
                    <button onClick={() => { setEditingBatchId(batch.batchId); setEditBatchData({ mailSubject: batch.mailSubject, mailContent: "", mailSignature: "" }); }}
                      className="p-1.5 text-muted-foreground hover:text-foreground hover:bg-secondary rounded-lg transition-colors border border-border"><Edit2 size={14} /></button>
                    <button onClick={() => handleDownloadPDF(batch)} className="p-1.5 text-muted-foreground hover:text-foreground hover:bg-secondary rounded-lg transition-colors border border-border"><FileDown size={14} /></button>
                    <button onClick={() => handleDeleteBatch(batch.batchId)} className="p-1.5 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-lg transition-colors border border-border"><Trash2 size={14} /></button>
                  </div>
                </div>
              )}
            </div>
      ...
      {!loading && viewMode === "emails" && (
        <div className="space-y-3">
          {selectedBatch && (
            <div className="bg-secondary border border-border rounded-xl px-4 py-3 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">{emails.length} email</p>
              </div>
              {selectedBatch.pendingCount > 0 && (
                <button onClick={() => handleSendBatchNow(selectedBatch.batchId)}
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-primary/10 text-primary border border-primary/20 rounded-lg text-sm hover:bg-primary/20 transition-colors">
                  <Zap size={13} />Tümünü Gönder
                </button>
              )}
            </div>
          )}
          ...
          {emails.length === 0 && (
            <div className="text-center py-16 text-muted-foreground bg-card border border-border rounded-xl">
              <Send size={36} className="mx-auto mb-3 opacity-20" /><p className="text-sm">Email yok</p>
            </div>
          )}

          {emails.map(email => (
            <div key={email.id} className="bg-card border border-border rounded-xl p-4">
              {editingId === email.id ? (
                <div className="space-y-3">
                  <input value={editData.recipientEmail || ""} onChange={e => setEditData(p => ({...p, recipientEmail: e.target.value}))}
                    className="w-full px-3 py-2 bg-input border border-border rounded-lg text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-ring" placeholder="Email" />
                  ...
                  <div className="flex gap-3">
                    <input type="date" value={editData.scheduledDate || ""} onChange={e => setEditData(p => ({...p, scheduledDate: e.target.value}))}
                      className="flex-1 px-3 py-2 bg-input border border-border rounded-lg text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-ring" />
                    <input type="time" value={editData.scheduledTime || ""} onChange={e => setEditData(p => ({...p, scheduledTime: e.target.value}))}
                      className="flex-1 px-3 py-2 bg-input border border-border rounded-lg text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-ring" />
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => handleSaveEdit(email.id)} className="px-3 py-1.5 bg-primary text-[oklch(0.11_0.005_260)] rounded-lg text-sm font-semibold hover:opacity-90">Kaydet</button>
                    <button onClick={() => setEditingId(null)} className="px-3 py-1.5 bg-secondary border border-border text-foreground rounded-lg text-sm hover:bg-muted">İptal</button>
                  </div>
                </div>
              ) : (
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{email.scheduledDate} {email.scheduledTime}</p>
                  </div>
                  <div className="flex gap-2 shrink-0">
                    <button onClick={() => { setEditingId(email.id); setEditData({ recipientEmail: email.recipientEmail, mailSubject: email.mailSubject, scheduledDate: email.scheduledDate, scheduledTime: email.scheduledTime }); }}
                      className="p-1.5 text-muted-foreground hover:text-foreground hover:bg-secondary rounded-lg transition-colors border border-border"><Edit2 size={13} /></button>
                    <button onClick={() => handleDelete(email.id)} className="p-1.5 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-lg transition-colors border border-border"><Trash2 size={13} /></button>
                    {email.status === "pending" && (
                      <button onClick={() => handleSendNow(email.id)} className="flex items-center gap-1 px-2.5 py-1.5 bg-primary/10 text-primary border border-primary/20 rounded-lg text-xs hover:bg-primary/20 transition-colors"><Zap size={12} />Gönder</button>
                    )}
                  </div>
                </div>
              )}
